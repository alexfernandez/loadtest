...
class NetworkClient {
	...
	parseBulk(bulk) {
		...
		const headerPart = string.substring(0, split)
		const body = string.substring(split + 4)
		...
		if (contentLength != body.length) {
			throw new Error(`Invalid content length ${contentLength}, body length ${body.length}`)
		}
		const result = {
			statusCode,
		}
