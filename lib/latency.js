...
export class Latency {
	...
	showPartial() {
		...
		if (!this.options.quiet) {
			console.info(`Requests: ${this.completedRequests}${requestPercent}, requests per second: ${result.rps}, mean latency: ${result.meanLatencyMs} ms`)
			if (this.totalErrors) {
				const errorPercent = Math.round(100 * 10 * this.totalErrors / this.completedRequests) / 10;
	...
	getRequestPercent() {
		...
		return ` (${Math.round(100 * this.completedRequests / this.options.maxRequests)}%)`
	}
	 ...
	 */
	shouldStop() {
		if (this.options.maxRequests && this.completedRequests >= this.options.maxRequests) {
			return true;
		}
