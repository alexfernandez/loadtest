...
exports.startServer = function(port, callback)
{
	if (!parseInt(port))
	{
		return error('Invalid port ' + port, callback);
	}
	...
	server.on('error', function(error)
	{
		...
		{
			return error('Port ' + port + ' in use, please free it and retry again', callback);
		}
		return error('Could not start server on port ' + port + ': ' + error, callback);
	});
