...
function HttpClient(operation, params)
{
	...
	self.start = function()
	{
		...
		if (params.rampUp)
		{
			startRampUp(params.requestsPerSecond, params.minRps, params.rampUp);
		} else {
			requestTimer = new timing.HighResolutionTimer(1000 / params.requestsPerSecond, self.makeRequest);
		}
	};

	function startRampUp(maxRps, minRps, rampUp) {
		var currentRps = minRps;
		var reqInterval = 1000 / currentRps;
		var diff = maxRps - minRps;
		var increase = diff / rampUp;
		var rampInterval = 1000;
		var requestTimer;
		var rampTimer;
		...
		requestTimer = new timing.HighResolutionTimer(reqInterval, self.makeRequest);
		setTimeout(function() {
			rampTimer = new timing.HighResolutionTimer(rampInterval, updateRps)
		}, rampInterval);

		function updateRps()
		{
			console.log('RPS Update Interval: ' + ((Date.now() - start) / 1000));
			currentRps = currentRps + increase;
			if (currentRps >= maxRps) {
				currentRps = maxRps;
				rampTimer.stop()
			}
			console.log('New RPS: ' + currentRps)
			reqInterval = 1000 / currentRps;
			requestTimer.stop();
			requestTimer = new timing.HighResolutionTimer(reqInterval, self.makeRequest);
		}
	}
