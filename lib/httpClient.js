...
class HttpClient {
	...
	start() {
		...
		if (this.params.rampUp) {
			this.startRampUp(this.params.requestsPerSecond, this.params.minRps, this.params.rampUp);
		} else {
			this.requestTimer = new timing.HighResolutionTimer(1000 / this.params.requestsPerSecond, this.makeRequest);
		}
	}

	startRampUp(maxRps, minRps, rampUp) {
		...
		this.requestTimer = new timing.HighResolutionTimer(reqInterval, this.makeRequest);
		setTimeout(function() {
