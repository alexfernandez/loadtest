...
class Latency {
	 ...
	 */
		...
		var elapsed = this.getElapsed(this.requests[requestId]);
	 ...
	 */
	showPartial() {
		var elapsedSeconds = this.getElapsed(this.lastShown) / 1000;
		var meanTime = this.partialTime / this.partialRequests || 0.0;
		var results = {
	...
	isFinished() {
		...
		var elapsedSeconds = this.getElapsed(this.initialTime) / 1000;
	...
	getResults() {
		var elapsedSeconds = this.getElapsed(this.initialTime) / 1000;
		var meanTime = this.totalTime / this.totalRequests;
		return {
...
function testLatencyRequests(callback) {
	...
	var errorCode = '500';
	var latency = new Latency(options, (error, result) => {
...
function testLatencyPercentiles(callback) {
	...
	var latency = new Latency(options, error => {
...
class HighResolutionTimer {
	constructor(delayMs, callback) {
		...
		// start timer
		this.delayed();
		this.timer = setTimeout(() => this.delayed(), this.delayMs);
	}
	...
	delayed() {
		...
		this.callback();
		this.counter ++;
		var diff = (Date.now() - this.start) - this.counter * this.delayMs;
		var timeout = Math.floor(this.delayMs - diff);
		if (timeout <= 0) {
			return this.delayed();
		}
	...
	traceDrift() {
		...
		var drift = diff / this.delayMs - this.counter;
...
function testTimer(callback) {
	var timer = new HighResolutionTimer(10, callback);
	timer.stop();
}
