 ...
 * An optional callback will be called with the results after max is reached.
 */
exports.Latency = function(options, callback)
{
	...
	function finish()
	{
		if (callback)
		{
			return callback(self.getResults());
		}
		self.show();
	}
	...
	{
		var secondsElapsed = (microtime.now() - initialTime) / 1000000;
	...
	self.computePercentiles = function()
	{
		...
		var counted = 0;
		for (var i = 0; i < maxLatencyMs; i++)
		{
			if (!histogramMs[i])
			{
				continue;
			}
			counted += histogramMs[i];
			var percent = counted / totalRequests;
