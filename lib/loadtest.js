	...
	function getConnect(id, callback)
	{
		return function(connection)
		{
			...
			connection.on('end', function(result)
			{
				...
				{
					return callback('Status code ' + connection.statusCode, statusCode);
				}
				callback(null, false);
			});
		};
...
var Operation = function(options, constructor, callback)
{
	...
	self.callback = function(error, result, next)
	{
		...
		{
			totalErrors += 1;
			var statusCode = '' + parseInt(result);
	...
	function stop()
	{
		...
		if (callback)
		{
			var results = self.latency.getResults();
			results.totalErrors = totalErrors;
	...
	function showPartial()
	{
		...
		if (totalErrors)
		{
			var percent = Math.round(100 * 10 * totalErrors / requests) / 10;
			log.info('Errors: %s, %s% of total requests', totalErrors, percent);
		}
	}
}
