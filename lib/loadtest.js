...
exports.loadTest = function(options, callback)
{
	...
	if (options.rampUp && options.minRps && options.requestsPerSecond)
	{
		// options.rampUp = options.rampUp / options.concurrency;
		options.minRps = (options.minRps < options.requestsPerSecond) ?
			options.minRps / options.concurrency :
	...
	{
		if (options.requestsPerSecond || options.minRps || options.rampUp)
		{
			log.error('"requestsPerSecond", "minRps" and "rampUp" not supported for WebSockets');
		}
	}
