...
export function loadTest(options, callback) {
	if (!options.url) {
		console.error('Missing URL in options');
		return;
	}
	...
	if (!options.url.startsWith('http://') && !options.url.startsWith('https://') && !options.url.startsWith('ws://')) {
		console.error('Invalid URL %s, must be http://, https:// or ws://', options.url);
		return;
	}

	if (options.url.startsWith('ws:')) {
		if (options.requestsPerSecond) {
			console.error('"requestsPerSecond" not supported for WebSockets');
		}
	}
