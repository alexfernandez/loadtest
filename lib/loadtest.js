...
class Operation {
	...
	start() {
		...
		if (this.options.maxSeconds) {
			this.stopTimeout = setTimeout(this.stop, this.options.maxSeconds * 1000);
		}
		...
		for (var index = 0; index < this.options.concurrency; index++) {
			if (this.options.indexParam) {
				...
				if(this.options.body) {
					var strBody = JSON.stringify(this.options.body);
			...
			var client = constructor(this, this.options);
	...
	stop() {
		...
		Object.keys(this.clients).forEach(function(index) {
			this.clients[index].stop();
		});
		if (this.callback) {
			var result = this.latency.getResults();
